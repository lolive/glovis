// NhapDataset.java defines the NHAP base class.
//----------------------------------------------
...
{
    private static final int[] resolutions = {90,30};
    ...
    (
        ...
        String sensorName,      // I: name to show for the sensor
        String inventoryDir,    // I: name of directory for the data
        int referencePoints     // I: number of reference points per degree
                                //    of latitude
    )
    {
        super(applet, sensorName, inventoryDir, "NHAP","showNhapBrowse.cgi",
              "showNhapMetadata.cgi","USGS_logo.gif",
              "http://www.usgs.gov",
              "https://lta.cr.usgs.gov/NHAP",
              null, "Enter a Full NHAP scene ID",resolutions,
              borderX, borderY, borderColor);

        // set the flags for the optional sensor features available
        isOrderable = true;            // can order different-micron scan
        hasCloudCover = false;
        hasProjectName = true;
        hasCustomSceneInfoLine = true;
        hasSecondaryIDMetadata = true;
        hideGridEntry = true;
        isDownloadable = true;         // web-enabled June 2008
        ...
        navModel = new NhapNavModel(referencePoints);
    }
    ...
    public String getCustomSceneInfo(Metadata scene)
    {
        ...
        String roll = scene.entityID.substring(length - 7, length - 3);
